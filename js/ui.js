    ...
    export function updateUIForSimulationState(isRunning, isSettingStart, trackLoaded, isCustomTrackActive) {
        ...
        elems.resetButton.disabled = isRunning || isSettingStart; // Allow reset even if track error
        elems.setStartPositionButton.disabled = isRunning || !trackLoaded;

        // Disable parameter inputs during simulation or when setting start position
        [ elems.timeStepInput, elems.maxRobotSpeedMPSInput, elems.motorResponseFactorInput,
          ...
          // Robot geom read-only for now in this view
          // elems.robotWidthInput_actual, elems.robotLengthInput_actual, elems.sideSensorSpreadInput, elems.sensorForwardOffsetInput, elems.sensorDiameterInput,
    ...
    export function setupTabNavigation() {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                // Optional: If canvases need resizing or re-rendering when tab becomes visible
                 if (targetTab === 'robotEditor' && domElements.robotEditorCanvas) {
                    // robotEditor.onTabVisible(); // Example: call a method in robotEditor.js
                 } else if (targetTab === 'trackEditor' && domElements.trackEditorCanvas) {
                    // trackEditor.onTabVisible(); // Example: call a method in trackEditor.js
                 }
            });
        });
    }
    ...
    export function getSimulationParameters() {
        const elems = getDOMElements();
        // Update currentMaxValAdjPID for bar display based on VELOCIDAD_BASE
        // This should ideally be done when VELOCIDAD_BASE changes.
        // For now, we'll read it here too.
        const baseSpeed = parseFloat(elems.arduinoVelBaseInput.value) || 110;
        // currentMaxValAdjPID = baseSpeed + 255; // Max positive or negative adjustment
        ...
        return {
            ...
            // Robot geometry is read from the robot object itself, but displayed here
            // For now, we are not allowing direct edit of geom here, it's from robotEditor
        };
    }