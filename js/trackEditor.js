...
// No longer need debugCellPathToDraw
...
let lastGeneratedTrackStartPosition = null; // Store start position from last generation
...
export function initTrackEditor(mainAppInterface) {
    ...
    elems.trackGridSize.addEventListener('change', (e) => {
        ...
        lastGeneratedTrackStartPosition = null; // Reset on grid size change
        setupGrid();
        renderEditor();
    });

    elems.generateRandomTrack.addEventListener('click', () => {
        lastGeneratedTrackStartPosition = null; // Reset before new generation
        generateRandomTrackWithRetry();
    });

    elems.exportTrackFromEditor.addEventListener('click', () => {
        if (!validateTrack()) {
            if (!confirm("La pista puede tener problemas. ¿Exportar de todos modos?")) {
                return;
            }
        }
        ...
        if (trackCanvas) {
            ...
            if (lastGeneratedTrackStartPosition) {
                ...
                // Default start if no specific generated start (e.g., manual edit or load)
    ...
    elems.loadTrackDesignInput.addEventListener('change', (event) => {
        lastGeneratedTrackStartPosition = null; // Clear generated start on load
        loadTrackDesign(event);
    });

    editorCanvas.addEventListener('click', (event) => {
        lastGeneratedTrackStartPosition = null; // Clear on manual edit
...
function setupGrid() {
    ...
    // debugCellPathToDraw = null; // Removed
    if (editorCanvas) {
...
function renderEditor() {
    // ... (Render logic now *only* draws grid and placed parts with connection indicators,
    //      the debugCellPathToDraw logic is removed for cleaner output) ...
...
function generateRandomTrackWithRetry(maxRetries = 10) {
    ...
    for (let i = 0; i < maxRetries; i++) {
        ...
        const generationResult = generateRandomLoopTrack(); // Get result object
        if (generationResult.success) {
            lastGeneratedTrackStartPosition = generationResult.startPosition; // Store it
            console.log(`Random Predefined Path track generated successfully on attempt ${i + 1}`);
            return; // Return void, success is handled by storing startPosition
        }
    ...
    lastGeneratedTrackStartPosition = null; // Ensure it's null on failure
    setupGrid();
    renderEditor();
}
...
function generateCellPathAndConnections() {
    // ... (same as before - this function should be robust enough now) ...
...
function generateRandomLoopTrack() {
    ...
    let placedCount = 0;
    let rectaSegments = []; // Store {r, c, rotation_deg} of placed "Recta" parts
        ...
        for (const partDef of shuffledLoopParts) {
            ...
            for (const rot of shuffledRotations) {
                ...
                if (match) {
                    ...
                    if (partDef.name.toLowerCase().includes("recta")) { // Check if it's a straight piece
                        rectaSegments.push({ r, c, rotation_deg: rot });
                    }
                    placedPiece = true;
                    placedCount++;
                    break; 
    ...
    if (allPartsPlaced && rectaSegments.length > 0) {
        ...
        let angle_rad = 0;
        if (randomRecta.rotation_deg === 0) angle_rad = -Math.PI / 2; // N-S straight, facing South (down)
        else if (randomRecta.rotation_deg === 90) angle_rad = 0;          // E-W straight, facing East (right)
        else if (randomRecta.rotation_deg === 180) angle_rad = Math.PI / 2; // S-N straight, facing North (up)
        else if (randomRecta.rotation_deg === 270) angle_rad = Math.PI;   // W-E straight, facing West (left)
        
        foundStartPosition = { r: randomRecta.r, c: randomRecta.c, angle_rad: angle_rad };
        console.log("Selected random start position on a 'Recta':", foundStartPosition);
    } else if (allPartsPlaced && cellPathWithConnections.length > 0) {
        // Fallback if no "Recta" found but track was placed: use first part of path
        const firstPathCell = cellPathWithConnections[0];
        const firstPart = grid[firstPathCell.r][firstPathCell.c];
        let angle_rad = 0; // Default angle
        if (firstPart) {
             // Determine a sensible angle based on its connections
            const conns = getRotatedConnections(firstPart, firstPart.rotation_deg);
            if (conns.S) angle_rad = -Math.PI / 2; // Facing South
            else if (conns.E) angle_rad = 0;       // Facing East
            else if (conns.N) angle_rad = Math.PI / 2; // Facing North
            else if (conns.W) angle_rad = Math.PI;   // Facing West
        }
        foundStartPosition = { r: firstPathCell.r, c: firstPathCell.c, angle_rad: angle_rad };
        console.log("No 'Recta' found, using first path segment for start position:", foundStartPosition);
    }
...
function validateTrack() {
    ...
    if (partCount === 0 && !debugCellPathToDraw) { alert("Validación: La pista está vacía."); return false; }
...
function loadTrackDesign(event) {
    // ... (same as before)
    // debugCellPathToDraw = null; // Removed, setupGrid handles it
